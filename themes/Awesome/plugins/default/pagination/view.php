...
if (count($pages) !== 1) {
	...
	<div class="container-table">
		<div class="center-row">
				<ul class="pagination ossn-pagination margin-top-20">';
  	//disaply first if first page is exist
	if(isset($first) && !empty($first)){
        $url = "?offset={$first}{$args_url}";
     	echo "<li class='page-item'><a class='page-link' href='{$url}' class='ossn-pagination-page'>".ossn_print('ossn:pagination:first')."</a></li>";	
	} 
   foreach ($pages as $page) {
        if ($page == $params['offset']) {
            $selected = 'active';
            $url = "?offset={$page}{$args_url}";
            echo "<li class='page-item {$selected}'><a class='page-link' href='{$url}'>{$page}</a></li>";
        } else {
            $url = "?offset={$page}{$args_url}";
